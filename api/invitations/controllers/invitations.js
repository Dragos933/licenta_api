...
module.exports = {
  find: async (ctx) => {
    ...
    if (ctx.query._q) {
      entities = await strapi.services.invitations.search(ctx.query);
    } else {
      entities = await strapi.services.invitations.find(ctx.query);
    }
    const toReturn = [];
    ...
    return toReturn
      .map(entity => sanitizeEntity(entity, { model: strapi.models.invitations }));
  },
};
